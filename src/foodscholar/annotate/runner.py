...
internally), then per-chunk embedding via the existing `SourceTypeRouter`.
The enriched chunks are upserted back to the chunk store in one bulk write
per batch. No I/O outside the stores it's handed.
"""
